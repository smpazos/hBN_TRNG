...
# ARDUINO supply voltage, from USB.
# Suggestion: Since there is a schottky diode in series between VDD line and 
# USB +5V, voltage drop may vary between boards.
# Measure with voltmeter for accuracy and update value as needed
VDD_ARDUINO = 4.75  
V_DIVIDER = 2       # Voltage divider implemented in the bread borad for voltage limitation.
bias_value = 0.36    # Sample initial bias voltage, sets the PWM output to bias the device.
bias_step = 0.01
thresh_step = 0.005


#============================================================================#
# A basic GUI to interact with setup parameters live:
# 1) Comparison threshold for reseeding
# 2) Applied bias to the device
def trngGui(outputs_dir,conn,bias_val,thres):
    sys.stdout = open(outputs_dir+'gui_log.txt', 'w')
    window = Tk()
    window.title("TRNG interactive controls")
    window.geometry('350x200')
    lbl = Label(window, text="Press!")
    lbl.grid(column=0, row=0)
    
    def thup():
        print("th +")
        lbl.configure(text="th + !!!")
        conn.send('+')
    ...
    def nlfsrstart():
        print("Run NLFSR")
        lbl.configure(text="Running")
        conn.send('L')
        
    btnTHUP = Button(window, text="th+", command=thup)
    btnTHUP.grid(column=1, row=0)
    btnTHDN = Button(window, text="th-", command=thdn)
    btnTHDN.grid(column=2, row=0)
    
    btnBUP = Button(window, text="bias +", command=biasup)
    btnBUP.grid(column=1, row=1)
    btnBDN = Button(window, text="bias -", command=biasdn)
    btnBDN.grid(column=2, row=1)
    
    btnNLFSR = Button(window, text="NLFSR run", command=nlfsrstart)
    btnNLFSR.grid(column=4, row=1)
...
def monitorArduino(clkslow,outputs_dir,conn):
    ...
    board = pyfirmata.Arduino('COM10') # Replace port number as needed
    ...
    while True:
        if conn.poll():
            ...
            if msg == 8:
                ...
                conn.send(12)
            if msg == 'p':    
                bias_voltage = bias_voltage + bias_step
                pwm_counter = bias_voltage/VDD_ARDUINO
                bias.write(pwm_counter)
                print('Bias voltage = '+str(bias_voltage))
            if msg == 'm':    
                bias_voltage = bias_voltage - bias_step
                pwm_counter = bias_voltage/VDD_ARDUINO
                bias.write(pwm_counter)
...
def nlfsrThread(state,count,outputs_dir,conn):
    ...
    if reseed != 3:     
        ...
        print('Saving .bmp...')
        im = Img.fromarray(array*255).convert('1')
...
def main():
    try:
        ...
        # sample = 0
        # Threshold level (of TIA output) for the analog input comparator        
        THRESHOLD_TIA = 0.5   # 2.5 for short meas
        THRESHOLD = THRESHOLD_TIA/V_DIVIDER/VDD_ARDUINO
        # Threshold limit (of TIA output) that triggers device RESET 
        SET = 6.5
        SET_LIMIT = SET/V_DIVIDER/VDD_ARDUINO
        ...
        # line3, = ax.plot([0,1400],[SET,SET],'g--')
        ...
        # Launch GUI thread and setup IPC
        parent_gui, child_gui = Pipe()
        guithread = Process(target=trngGui, args=(outputs_dir,child_gui,bias_value,THRESHOLD_TIA,))
        guithread.start()
        GUI_running = True
        print('GUI launched.')
        ...
        while True:
            ...
            # Poll GUI process for pressed buttons
            if parent_gui.poll():
                guimsg = parent_gui.recv()
                if guimsg == '+':
                    THRESHOLD = THRESHOLD + thresh_step
                    THRESHOLD_TIA = THRESHOLD*V_DIVIDER*VDD_ARDUINO
                    print('Threshold increased to ' + str(THRESHOLD_TIA))
                    print("Threshold = ", THRESHOLD_TIA, tn,file = tia_log)
                if guimsg == '-':
                    THRESHOLD = THRESHOLD - thresh_step
                    THRESHOLD_TIA = THRESHOLD*V_DIVIDER*VDD_ARDUINO
                    print('Threshold decreased to ' + str(THRESHOLD_TIA))
                    print("Threshold = ", THRESHOLD_TIA, tn,file = tia_log)
                if guimsg == 'p':
                    bias_local = bias_local + bias_step
                    print('Bias increased to ' + str(bias_local))
                    parent_conn_ard.send('p')
                    print("Bias", bias_local, tn,file = tia_log)
                if guimsg == 'm':
                    bias_local = bias_local - bias_step
                    print('Bias decreased to ' + str(bias_local))
                    parent_conn_ard.send('m')
                    print("Bias = ",bias_local, tn,file = tia_log)
                if guimsg == 'L':
                    print('NLFSR launched at t=' + str(time.time()-t0))
                    parent_conn.send('L')
                    print('NLFSR launched at t=' + str(time.time()-t0),file = tia_log)
                    nlfsr_running = True
            ...
            if nlfsr_running:
                ...
                if parent_conn.poll():
                    if parent_conn.recv() == 5:
                        print('NLFSR completed requested periods and is shutting down. Elapsed time was:')
                        print(time.time()-t0)
                        nlfsr_running = False
                        break
